!function () {
    ...
    function move(curEle, target, duration,effect,callback) {  //用来驱动的
        ...
        for (var key in target) {//我们需要通过target的属性多少来查找我们begin的属性，因为如果不是target的属性，我们这个维度就没有必要获取了
        ...
        //以上我们只是获取了动画刚开始需要的一些条件 , t ,b ,c ,d
        //接下来需要我们开启一个定时器
        curEle.timer = window.setInterval(function () {
            //1 越界判断 2  设置每个维度的值

            time += interval;//通过花费时间的增长来驱动对应时间走过的距离
            if(time>=duration){
                window.clearInterval(curEle.timer);
                //我们还要赋值到target终点
            ...
            //需要分别处理每一个维度,targe里面的维度都需要处理
            for(var k in target){
                if(target.hasOwnProperty(k)){
                    //这里所有能遍历出来的维度都需要处理
                    var curPosi = tempEffect(time,begin[k],change[k],duration);
                    utils.css(curEle,k,curPosi/*维度*//*维度的值*/);
                }
            }
        }, interval)
    }
